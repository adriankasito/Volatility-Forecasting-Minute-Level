...
class GarchModel:
    ...
    def wrangle_data(self, n_observations):
        ...
        if self.use_new_data:
            ...
            api = AlphaVantageAPI()
            #Get data
            new_data = api.get_daily(self.ticker)
    ...
    def __clean_prediction(self, prediction):
        ...
        -------
        dict
            Forecast of volatility. Each key is timestamp in ISO 8601 format.
            Each value is predicted volatility.
        """
        # Calculate forecast start timestamp
        start = prediction.index[0] + pd.DateOffset(minutes=1)

        # Create timestamp range at minute intervals
        prediction_timestamps = pd.date_range(start=start, periods=prediction.shape[1], freq='T')

        # Create prediction index labels, ISO 8601 format
        prediction_index = [ts.isoformat() for ts in prediction_timestamps]
